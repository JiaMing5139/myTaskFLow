...
class FlowBuilder {
  ...
  public:
    ...
    */
    template <typename C>
    std::enable_if_t<is_static_task_t<C>::v, Task> emplace(C&& callable);
    ...
    */
    template <typename C>
    std::enable_if_t<is_dynamic_task_t<C>::v, Task> emplace(C&& callable);
    ...
    */
    template <typename C>
    std::enable_if_t<is_condition_task_t<C>::v, Task> emplace(C&& callable);
...
//auto FlowBuilder::emplace(C&&... cs)-> decltype(std::make_tuple(emplace(std::forward<C>(cs))...))  {
...
template <typename C>
std::enable_if_t<is_static_task_t<C>::v, Task> FlowBuilder::emplace(C&& c) {
...
template <typename C>
std::enable_if_t<is_dynamic_task_t<C>::v, Task> FlowBuilder::emplace(C&& c) {
  auto n = _graph.emplace_back(
...
template <typename C>
std::enable_if_t<is_condition_task_t<C>::v, Task> FlowBuilder::emplace(C&& c) {
  auto n = _graph.emplace_back(
...
//    auto operator()(const para1& l, const para2& r) -> decltype(std::min(l, r)){
//        return std::min(l, r);
//    }
//};
//
...
//public:
//    template <typename para1,typename para2>
//    auto operator()(const para1& l, const para2& r) -> decltype(std::max(l, r)){
//        return std::max(l, r);
//    }
//};
//
...
//template <typename I, typename T>
//std::pair<Task, Task> FlowBuilder::reduce_min(I beg, I end, T& result) {
//  return reduce(beg, end, result,cpp11Min());
//}
//
//// Function: reduce_max
//// Find the maximum element over a range of items.
//template <typename I, typename T>
//std::pair<Task, Task> FlowBuilder::reduce_max(I beg, I end, T& result) {
//  return reduce(beg, end, result, cpp11Max());
//}
