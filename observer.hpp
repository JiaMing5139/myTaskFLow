...
*/
class ChromeMutiTaskOberver: public ObserverInterface{
...
private:
    ...
    std::vector<Timeline> _timeline;
};

    void ChromeMutiTaskOberver::set_up(size_t num_workers) {

    }

    void ChromeMutiTaskOberver::on_exit(size_t worker_id, TaskView task_view) {

    }

    void ChromeMutiTaskOberver::on_entry(size_t worker_id, TaskView task_view) {

    }

    size_t ChromeMutiTaskOberver::num_tasks() const {
        return 0;
    }

    void ChromeMutiTaskOberver::clear() {

    }

    std::string ChromeMutiTaskOberver::dump() const {
        return std::string();
    }

    void ChromeMutiTaskOberver::dump(std::ostream &ostream) const {

    }

    ChromeMutiTaskOberver::Segment::Segment(const std::string &n,
                                            std::chrono::time_point<std::chrono::steady_clock> b) {
    ...
    ChromeMutiTaskOberver::Segment::Segment(const std::string &n, std::chrono::time_point<std::chrono::steady_clock> b,
                                            std::chrono::time_point<std::chrono::steady_clock> e) {
...
public:
    ...
    auto operator() (size_t sum,const para& exe) -> decltype(sum + exe.size()){
        return sum + exe.size();
    }
};
// FIXME
