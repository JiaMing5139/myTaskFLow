...
class Executor {
  ...
  public:
    ...
    */
    std::future<Timeline> run(Taskflow& taskflow);
    ...
    */
    template<typename C>
    std::future<Timeline> run(Taskflow& taskflow, C&& callable);
    ...
    @return a std::future to access the execution status of the taskflow
    */
    std::future<Timeline> run_n(Taskflow& taskflow, size_t N);
    ...
    */
    template<typename C>
    std::future<Timeline> run_n(Taskflow& taskflow, size_t N, C&& callable);
    ...
    */
    template<typename P>
    std::future<Timeline> run_until(Taskflow& taskflow, P&& pred);
    ...
    */
    ...
    std::future<Timeline> run_until(Taskflow& taskflow, P&& pred, C&& callable);
  ...
  private:
    ...
    std::vector<tf::Topology*> _running_topologies_;
...
inline bool Executor::_wait_for_task(Worker& worker, Node*& t) {
  ...
  LOG_TRACE <<  "_num_thieves:" << _num_thieves[d];
  if(t) {   //偷到了任务
     LOG_TRACE << "get task from explore task queue";
    if(_num_thieves[d].fetch_sub(1,std::__1::memory_order_seq_cst) == 1) {
        LOG_TRACE << "_num_thieves[d].fetch_sub(1) == 1";
        LOG_TRACE << "_notifier[d].notify(false);";
      _notifier[d].notify(false);
    }
    return true;
  }

  LOG_TRACE << "prepare_wait";
  _notifier[d].prepare_wait(worker.waiter);
  
  //if(auto vtm = _find_vtm(me); vtm != _workers.size()) {
  if(!_wsq[d].empty()) {
      LOG_TRACE << "exector wsq is not empty";
      LOG_TRACE << "_notifier[d].cancel_wait(worker.waiter)";
    _notifier[d].cancel_wait(worker.waiter);
    //t = (vtm == me) ? _wsq.steal() : _workers[vtm].wsq.steal();
    
    t = _wsq[d].steal();  // must steal here
    if(t) {
        LOG_TRACE << "get task from exector's task queue";
...
inline void Executor::_invoke_static_work(Worker& worker, Node* node) {
 // _observer_prologue(worker, node);
  node->_topology->on_entry(worker.id,node);
  nstd::get<Node::StaticWork>(node->_handle).work();
  node->_topology->on_exit(worker.id,node);
...
// Function: run
inline std::future<Timeline> Executor::run(Taskflow& f) {
  return run_n(f, 1, [](){});
}

// Function: run
template <typename C>
std::future<Timeline> Executor::run(Taskflow& f, C&& c) {
  return run_n(f, 1, std::forward<C>(c));
}

// Function: run_n
inline std::future<Timeline> Executor::run_n(Taskflow& f, size_t repeat) {
  return run_n(f, repeat, [](){});
}

// Function: run_n
template <typename C>
std::future<Timeline> Executor::run_n(Taskflow& f, size_t repeat, C&& c) {
  return run_until(f, [repeat]() mutable { return repeat-- == 0; }, std::forward<C>(c));
}

// Function: run_until    
template<typename P>
std::future<Timeline> Executor::run_until(Taskflow& f, P&& pred) {
...
inline void Executor::_tear_down_topology(Topology* tpg) {
  ...
  // case 2: the final run of this topology
    ...
    if(f._topologies.size() > 1) {
      ...
      // Set the promise
      tpg->_promise.set_value(tpg->_timeline);
      ...
      p.set_value(std::move(tpg->_timeline));
...
// Function: run_until
template <typename P, typename C>
std::future<Timeline> Executor::run_until(Taskflow& f, P&& pred, C&& c) {

  _increment_topology();

  // Special case of predicate
  if(f.empty() || pred()) {
    std::promise<Timeline> promise;
    promise.set_value(Timeline());
    _decrement_topology_and_notify();
 ...
 // std::future<void> future;
   std::future<Timeline> future;
  {
    ...
    _running_topologies_.push_back(tpg); //为了监控任务
